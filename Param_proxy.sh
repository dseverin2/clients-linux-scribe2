...
if grep "use_proxy=on" /etc/wgetrc; then
	sudo sed '/[ht|f]tps\?_proxy*/d' /etc/wgetrc | tee /etc/wgetrc  # Nettoyage en cas de réintégration directe
fi
...
host='$proxy_def_ip'
port=$proxy_def_port" | sudo tee /usr/share/glib-2.0/schemas/my-defaults.gschema.override 2>> $logfile

	sudo glib-compile-schemas /usr/share/glib-2.0/schemas 2>> $logfile

	#Paramétrage du Proxy pour le système
	######################################################################
	writelog "---Inscription du proxy dans /etc/environment"
	sudo sed '/[h|f|n][a-z]*_proxy*/d' /etc/environment | sudo tee /etc/environment # Nettoyage en cas de réintégration directe
...
Acquire::https::proxy \"https://$scribeuserapt:$scribepass@$proxy_def_ip:$proxy_alt_port/\";" | sudo tee /etc/apt/apt.conf.d/20proxy 2>>$logfile
	...
	sudo snap set system proxy.http="http://$proxy_def_ip:$proxy_def_port"
	sudo snap set system proxy.https="http://$proxy_def_ip:$proxy_def_port"
fi
